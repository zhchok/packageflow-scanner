...
class ScannerSecurityTests(unittest.TestCase):
    ...
    def test_local_storage_contains_only_camera_preference(self) -> None:
        app = (ROOT / "app.js").read_text(encoding="utf-8")

        # ID камеры не содержит пользовательских или логистических данных.
        # Ограничиваем хранилище одним ключом, чтобы позже туда случайно не
        # начали записывать треки, Telegram ID или содержимое посылок.
        self.assertEqual(app.count("localStorage.getItem("), 1)
        self.assertEqual(app.count("localStorage.setItem("), 1)
        self.assertIn(
            'PREFERRED_CAMERA_STORAGE_KEY = "packageflow.preferred-camera.v1"',
            app,
        )
